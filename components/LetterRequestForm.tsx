...
const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement>> = ({ className, ...props }) => (
  <input className={`mt-1 flex h-8 sm:h-9 md:h-10 lg:h-11 xl:h-12 w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-2 sm:px-3 py-1 sm:py-2 text-xs sm:text-sm md:text-base ring-offset-white file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-gray-500 dark:placeholder:text-gray-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 transition-all duration-200 ${className}`} {...props} />
);

const Select: React.FC<React.SelectHTMLAttributes<HTMLSelectElement>> = ({ className, children, ...props }) => (
    <select className={`mt-1 flex h-8 sm:h-9 md:h-10 lg:h-11 xl:h-12 w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-2 sm:px-3 py-1 sm:py-2 text-xs sm:text-sm md:text-base ring-offset-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 transition-all duration-200 ${className}`} {...props}>
        {children}
    </select>
);

const Textarea: React.FC<React.TextareaHTMLAttributes<HTMLTextAreaElement>> = ({ className, ...props }) => (
    <textarea className={`mt-1 flex min-h-[60px] sm:min-h-[80px] md:min-h-[100px] w-full rounded-md border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 px-2 sm:px-3 py-1 sm:py-2 text-xs sm:text-sm md:text-base ring-offset-white placeholder:text-gray-500 dark:placeholder:text-gray-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 transition-all duration-200 ${className}`} {...props} />
);
...
export const LetterRequestForm: React.FC<LetterRequestFormProps> = ({ onFormSubmit, onCancel, letterToEdit }) => {
  ...
  return (
    <div className="w-full max-w-[320px] sm:max-w-2xl md:max-w-4xl lg:max-w-6xl xl:max-w-7xl mx-auto p-2 sm:p-4 lg:p-6 xl:p-8">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg sm:text-xl md:text-2xl">{letterToEdit ? 'Edit Letter' : 'Create a New Letter'}</CardTitle>
          <CardDescription className="text-xs sm:text-sm md:text-base">Fill in the details below to generate an AI-powered draft.</CardDescription>
        </CardHeader>
        ...
        <CardFooter className="flex flex-col sm:flex-row justify-between gap-3 sm:gap-4 px-3 sm:px-4 lg:px-6">
          <Tooltip text="Cancel and return to dashboard">
            <button onClick={onCancel} className="text-sm font-medium text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white transition-colors duration-200">
              Cancel
            </button>
          </Tooltip>
          {isGenerating ? (
            ...
            <Tooltip text="Use AI to generate a professional legal letter based on your inputs">
              <ShimmerButton onClick={handleGenerateDraft} className="h-8 sm:h-9 md:h-10 lg:h-11">
                Generate AI Draft
              </ShimmerButton>
            </Tooltip>
          )}
      ...
      <div className="lg:sticky lg:top-24">
        <Card>
          <CardHeader>
            <CardTitle className="text-base sm:text-lg md:text-xl">AI Draft Preview</CardTitle>
            <CardDescription className="text-xs sm:text-sm">The AI-generated content will appear here for your review.</CardDescription>
          </CardHeader>
