...
export const AdminDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'users' | 'employees'>('overview');
      ...
      if (usersError) {
        ...
        const processedUsers = await Promise.all(
          usersData.map(async (user) => {
            ...
            const totalSpent = subscriptions?.reduce((sum, sub) => sum + parseFloat(sub.amount.toString()), 0) || 0;
        ...
        const employeesWithAnalytics = await Promise.all(
          employeesData.map(async (employee) => {
            const { data: analytics } = await supabase
              .rpc('get_employee_analytics', { employee_uuid: employee.id });
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div className="bg-blue-50 rounded-xl p-6 border border-blue-200">
                      ...
                      <p className="text-3xl font-bold text-blue-900">{metrics.total_users}</p>
                    </div>
                    ...
                    <div className="bg-purple-50 rounded-xl p-6 border border-purple-200">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="text-sm font-medium text-purple-600">Total Letters</h3>
                        <FileText className="w-6 h-6 text-purple-600" />
                      </div>
                      <p className="text-3xl font-bold text-purple-900">{metrics.total_letters}</p>
                    </div>

                    <div className="bg-orange-50 rounded-xl p-6 border border-orange-200">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="text-sm font-medium text-orange-600">Employees</h3>
                        <UserCheck className="w-6 h-6 text-orange-600" />
                      </div>
                      <p className="text-3xl font-bold text-orange-900">{metrics.total_employees}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <div className="bg-white rounded-xl p-6 border border-gray-200">
                      ...
                      <p className="text-2xl font-bold text-gray-900">{metrics.active_subscriptions}</p>
                    </div>
                    ...
                    <div className="bg-white rounded-xl p-6 border border-gray-200">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="text-sm font-medium text-gray-600">Pending Letters</h3>
                        <Calendar className="w-6 h-6 text-gray-600" />
                      </div>
                      <p className="text-2xl font-bold text-gray-900">{metrics.pending_letters}</p>
                    </div>

                    <div className="bg-white rounded-xl p-6 border border-gray-200">
                      <div className="flex items-center justify-between mb-4">
                        <h3 className="text-sm font-medium text-gray-600">Completed Letters</h3>
                        <CheckCircle className="w-6 h-6 text-gray-600" />
                      </div>
                      <p className="text-2xl font-bold text-gray-900">{metrics.completed_letters}</p>
                    </div>
                  </div>
                </div>
              )}
                        ...
                        <div className="p-6">
                          ...
                          <AnimatePresence>
                            {selectedUserId === user.id && (
                              ...
                              >
                                <h4 className="text-md font-semibold text-gray-900 mb-4">Letters</h4>
                                <div className="space-y-3">
                                  {getUserLetters(user.id).map(letter => (
                                    <div key={letter.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                                      <div className="flex-1">
                                        <h5 className="font-medium text-gray-900">{letter.title}</h5>
                                  ...
                                  {getUserLetters(user.id).length === 0 && (
                                    <p className="text-gray-500 text-center py-4">No letters found</p>
                                  )}
                                </div>
                              </motion.div>
                            )}
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {employees.map(employee => (
                      ...
                      >
                        <div className="flex items-center justify-between mb-4">
                          <div>
                            <h3 className="text-lg font-semibold text-gray-900">{employee.email}</h3>
                        ...
                        <div className="bg-blue-50 rounded-lg p-4 mb-4">
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              <p className="text-xl font-bold text-blue-900">{employee.coupon_code}</p>
                            </div>
                            <div className="text-right">
                              <p className="text-sm text-blue-600">Used</p>
                              <p className="text-xl font-bold text-blue-900">{employee.coupon_usage_count}x</p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="text-center p-3 bg-green-50 rounded-lg">
                            ...
                            <p className="text-xl font-bold text-green-900">{employee.total_referrals}</p>
                          </div>
                          ...
                          <div className="text-center p-3 bg-purple-50 rounded-lg">
                            ...
                            <p className="text-sm text-purple-600">Total Commissions</p>
                          ...
                          <div className="text-center p-3 bg-orange-50 rounded-lg">
                            ...
                            <p className="text-sm text-orange-600">Current Points</p>
                            <p className="text-xl font-bold text-orange-900">{employee.current_points}</p>
                          </div>
                          ...
                          <div className="text-center p-3 bg-gray-50 rounded-lg">
                            ...
                            <p className="text-xl font-bold text-gray-900">{employee.monthly_referrals}</p>
                          </div>
                        </div>
