...
import { Eye, Edit, Trash2, FileText, Download, Send, Clock, CheckCircle, XCircle } from "lucide-react";
...
export function LettersTable({ 
  ...
  const filteredData = letters.filter((letter) => {
    return (
      ...
      (!titleFilter || letter.title.toLowerCase().includes(titleFilter.toLowerCase()))
    );
  });
  ...
  return (
    <div className="container my-4 space-y-4 p-6 border border-slate-200 dark:border-slate-800 rounded-lg bg-white dark:bg-slate-900 shadow-sm overflow-x-auto">
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">My Letter Requests</h2>
      ...
      <Table className="w-full">
        <TableHeader>
          <TableRow>
            {visibleColumns.includes("Title") && <TableHead className="w-[200px]">Letter Title</TableHead>}
            {visibleColumns.includes("Template") && <TableHead className="w-[180px]">Template Type</TableHead>}
            {visibleColumns.includes("Status") && <TableHead className="w-[120px]">Status</TableHead>}
            {visibleColumns.includes("Created") && <TableHead className="w-[120px]">Created</TableHead>}
            {visibleColumns.includes("Updated") && <TableHead className="w-[120px]">Updated</TableHead>}
            {visibleColumns.includes("Actions") && <TableHead className="w-[150px]">Actions</TableHead>}
          </TableRow>
        </TableHeader>
        <TableBody>
          {filteredData.length ? (
            filteredData.map((letter) => {
              ...
              return (
                <TableRow key={letter.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  {visibleColumns.includes("Title") && (
                    <TableCell className="font-medium">
                      <div className="flex items-center gap-2">
                        <FileText className="h-4 w-4 text-blue-600 dark:text-blue-400" />
                        <span className="truncate max-w-[150px]" title={letter.title}>
                          {letter.title}
                        </span>
                      </div>
                    </TableCell>
                  ...
                  {visibleColumns.includes("Status") && (
                    <TableCell>
                      <Badge className={cn("flex items-center gap-1.5", style.bg, style.text)}>
