...
export const AuthPage: React.FC<AuthPageProps> = ({ initialView = 'login', onBackToLanding }) => {
  ...
  const renderContent = () => {
        ...
        return (
            <>
                <CardHeader className="text-center">
                    ...
                    <CardDescription className="text-sm sm:text-base">Enter your email to receive a password reset link.</CardDescription>
                </CardHeader>
                    ...
                    <CardFooter className="flex flex-col gap-4 px-6">
                        ...
                        {loading ? (
                            <Tooltip text="Processing your password reset request...">
                              <ShinyButton disabled className="w-full h-10 sm:h-11">Processing...</ShinyButton>
                            </Tooltip>
                        ) : (
                            <Tooltip text="Click to receive a password reset link via email">
                              <ShimmerButton type="submit" className="w-full h-10 sm:h-11" disabled={!email || !!emailError}>
                                  Send Reset Link
                              </ShimmerButton>
                            </Tooltip>
                        )}
                        ...
                        <Tooltip text="Return to the sign in page">
                          <button type="button" onClick={() => switchView('login')} className="w-full text-sm text-blue-600 hover:underline dark:text-blue-400 transition-colors duration-200 text-center">
                              Back to Sign In
                          </button>
                        </Tooltip>
                    </CardFooter>
    ...
    return (
        <>
            <CardHeader className="text-center">
              <CardTitle className="text-xl sm:text-2xl">{view === 'login' ? 'Welcome Back' : 'Create an Account'}</CardTitle>
                    ...
                    <div className="flex justify-between items-baseline">
                        ...
                        {view === 'login' && (
                            <Tooltip text="Click to reset your password via email">
                              <button type="button" onClick={() => switchView('forgot_password')} className="text-xs text-blue-600 hover:underline dark:text-blue-400 transition-colors duration-200">Forgot Password?</button>
                            </Tooltip>
                        )}
                    </div>
              ...
              <CardFooter className="flex flex-col gap-4 px-6">
                ...
                {loading ? (
                    ...
                    <Tooltip text={view === 'login' ? 'Click to sign in to your account' : 'Click to create your new account'}>
                      <ShimmerButton type="submit" className="w-full h-10 sm:h-11" disabled={!email || !password || !!emailError}>
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-gray-950 flex flex-col items-center justify-center p-3 sm:p-4 lg:p-6 xl:p-8">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-2 sm:gap-3 mb-4 sm:mb-6 lg:mb-8">
        ...
        <span className="text-lg sm:text-xl lg:text-2xl xl:text-3xl font-bold text-gray-900 dark:text-white text-center">Law Letter AI</span>
      </div>
