...
export const EmployeeDashboard: React.FC = () => {
  ...
  const fetchEmployeeAnalyticsWithPoints = async (employeeId: string): Promise<EmployeeAnalyticsWithPoints> => {
    // This would normally be a function in discountService, but for now we'll simulate it
    const basicAnalytics = await discountService.getEmployeeAnalytics(employeeId);
  ...
  const handleGenerateCode = async () => {
    ...
    try {
      ...
      if (newCode) {
        ...
        showError('Generation Failed', 'Unable to generate discount code. Please try again.');
      }
    } catch (error) {
  ...
  const getPointsLevel = (points: number): { level: string; color: string; nextLevel: number } => {
    if (points < 5) return { level: 'Bronze', color: 'text-amber-600', nextLevel: 5 };
    if (points < 15) return { level: 'Silver', color: 'text-gray-500', nextLevel: 15 };
    if (points < 30) return { level: 'Gold', color: 'text-yellow-500', nextLevel: 30 };
    if (points < 50) return { level: 'Platinum', color: 'text-indigo-600', nextLevel: 50 };
      ...
      <div className="space-y-6">
        ...
        <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg border p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            <div className="text-center">
              <div className="text-3xl font-bold text-green-600">{analytics.totalReferrals}</div>
            ...
            <div className="text-center">
              <div className="text-3xl font-bold text-orange-600">{analytics.activeDiscountCodes}</div>
        ...
        <div className="bg-white rounded-lg border p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center">
              ...
              <h3 className="text-xl font-semibold text-gray-800">Points & Level</h3>
            </div>
            <div className={`px-4 py-2 rounded-full font-bold ${pointsLevel.color} bg-gray-100`}>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <span className="font-bold text-2xl">{analytics.totalPoints}</span>
            </div>
            ...
            {pointsLevel.nextLevel > 0 && (
              <>
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold">{pointsLevel.nextLevel - analytics.totalPoints}</span>
                </div>
            ...
            <div className="mt-4 p-4 bg-blue-50 rounded-lg">
              ...
              <ul className="text-sm text-blue-800 space-y-1">
                <li>• 1 point for each successful referral (user subscribes with your code)</li>
            ...
            {analytics.monthlyPoints > 0 && (
              <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                <span className="text-green-800">Points earned this month:</span>
                <span className="font-bold text-green-600">{analytics.monthlyPoints}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg border p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-xl font-semibold text-gray-800">Discount Codes</h3>
            <button
            ...
            <table className="w-full table-auto">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="px-4 py-2 text-left font-medium text-gray-700">Usage Count</th>
              ...
              <tbody>
                {discountCodes.map((code) => (
                  <tr key={code.id} className="border-t">
                    <td className="px-4 py-2 font-mono font-bold text-blue-600">{code.code}</td>
          ...
          {discountCodes.length === 0 && (
            <div className="text-center py-8 text-gray-500">
              No discount codes generated yet. Click "Generate New Code" to create your first one.
            </div>
          )}
        </div>
        ...
        {analytics.codeUsageStats.length > 0 && (
          <div className="bg-white rounded-lg border p-6">
            <h3 className="text-xl font-semibold text-gray-800 mb-4">Code Performance</h3>
            <div className="overflow-x-auto">
              <table className="w-full table-auto">
                <thead>
                  <tr className="bg-gray-50">
                    <th className="px-4 py-2 text-left font-medium text-gray-700">Code</th>
                    <th className="px-4 py-2 text-left font-medium text-gray-700">Uses</th>
                    <th className="px-4 py-2 text-left font-medium text-gray-700">Revenue Generated</th>
                    <th className="px-4 py-2 text-left font-medium text-gray-700">Your Commission</th>
                    <th className="px-4 py-2 text-left font-medium text-gray-700">Points Earned</th>
                  </tr>
                </thead>
                <tbody>
                  {analytics.codeUsageStats.map((stat) => (
                    <tr key={stat.code} className="border-t">
                      <td className="px-4 py-2 font-mono font-bold text-blue-600">{stat.code}</td>
                      <td className="px-4 py-2">{stat.usageCount}</td>
                      <td className="px-4 py-2">${stat.totalRevenue.toFixed(2)}</td>
                      ...
                      <td className="px-4 py-2">
                        <div className="flex items-center">
                          ...
                          <span className="font-bold text-yellow-600">{stat.usageCount}</span>
                        </div>
                      </td>
                    </tr>
                  ))}
