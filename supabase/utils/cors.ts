...
export function createJsonResponse(data: unknown, status = 200, requestOrigin?: string): Response {
  const headers = getCorsHeaders(requestOrigin);
...
export function createErrorResponse(error: unknown, status = 500, requestOrigin?: string): Response {
  const message =
