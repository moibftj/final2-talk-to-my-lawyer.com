...
async function handleSubscriptionCreated(
  ...
  if (discountCode) {
    await processDiscountUsage(supabase, discountCode, userId, subscription.id, priceItem.price.unit_amount / 100);
  }
...
async function processDiscountUsage(
  ...
  try {
    ...
    if (discount.employee_id) {
      ...
      console.log(`Commission calculated: $${commission} for employee ${discount.employee_id}`);
    }
  } catch (error) {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const event: StripeWebhookEvent = await verifyStripeSignature(req, config.webhookSecret);
    ...
    switch (event.type) {
      case 'customer.subscription.created':
        await handleSubscriptionCreated(supabase, event.data.object as SubscriptionData);
        break;

      case 'customer.subscription.updated':
        await handleSubscriptionUpdated(supabase, event.data.object as SubscriptionData);
        break;

      case 'customer.subscription.deleted':
        await handleSubscriptionDeleted(supabase, event.data.object as SubscriptionData);
        break;
